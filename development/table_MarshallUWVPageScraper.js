...
const scraperObject = {
  ...
  url: "https://www.marshall.edu/purchasing/resources/bids-and-proposals/",
  site_name: "MarshallUWV",
  async scraper(browser) {
    ...
    async function scrapeCurrentPage() {
      let titles = await page.$$eval(
        ...
        (title) => {
          title = title.map((element) => element.previousSibling.textContent);
          return title;
        }
      );
      ...
      for (title in titles) {
        try {
          ...
          let newOpen = isValidOpenDate(openDates[title]);
          let newClose = isValidCloseDate(closeDates[title]);
          ...
          fs.writeFile(
            "./0.Scraper2/MarshallUWVAllData.json",
            ...
            function (err) {
              ...
              console.log(
                "The data has been scraped and saved successfully! View it at '/0.Scraper2/MarshallUWVAllData.json'"
              );
            }
          );
        } catch (e) {
